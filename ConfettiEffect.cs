   ...
   public class ConfettiEffect
{
    ...
    public int ConfettiPerFrame { get; set; } = 20;
    ...
    private int confettiCreated;

    private Canvas confettiCanvas;

    public void StartConfettiEffect(Canvas canvas)
    {
    ...
    {
        int confettiToCreate = Math.Min(ConfettiPerFrame, ConfettiCount - confettiCreated);

        for (int i = 0; i < confettiToCreate; i++)
        {
        ...
        confettiCreated += confettiToCreate;

        if (confettiCreated >= ConfettiCount)
        {
            CompositionTarget.Rendering -= CompositionTarget_Rendering;
        }
    }
